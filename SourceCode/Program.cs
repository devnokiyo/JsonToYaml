        ...
        static void Main(string[] args)
        {
            ...
            foreach (var filePath in filePaths.Skip(1))
            {
                ...
                try
                {
                    ...
                    var readJsonObject = JsonConvert.DeserializeObject<ExpandoObject>(json, new ExpandoObjectConverter());
                    var writeYamlObject = new ExpandoObject();
                    (writeYamlObject as IDictionary<string, object>).Add(fileNameWithoutExtension, readJsonObject);

                    var yaml = serializer.Serialize(writeYamlObject);
